...
const DoctorsSection = ({
  surgeryKey,       // ✅ pass the "key" from surgeryData
  surgeryName,
  speciality,
  cities = ['Jaipur', 'Mumbai', 'Bangalore', 'Delhi'],
  defaultCity = 'Jaipur'
}) => {
  ...
  // ✅ Step 1 — Filter doctors by surgery key
  const doctorsForTreatment = doctorDummyData.filter(doc =>
    doc.topTreatments.some(t => t.key === surgeryKey)
  );

  // ✅ Step 2 — Apply search and city filter
  const filteredDoctors = doctorsForTreatment.filter(doc => {
    const nameMatch = doc.name.toLowerCase().includes(search.toLowerCase());
    const cityMatch = isExploreOtherCities
      ? !cities.includes(doc.city)
        ...
        <div className="flex-1">
          ...
          <div className="relative mb-4">
            <input
              ...
              placeholder={`Search Doctors for ${surgeryKey} Treatment`}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {[...cities, 'Explore other cities'].map((city, idx) => (
              <button
                key={idx}
            ...
            {filteredDoctors.length === 0 ? (
              ...
              filteredDoctors.map((doctor) => (
                <Link key={doctor.id} href={`/doctor/${doctor.id}`}>
                  <div className="bg-white rounded-xl shadow-md hover:scale-101 p-4 flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <p className="text-[10px] text-[#A0A0A0]">
                        Specialist in {doctor.speciality || speciality}
                      </p>
